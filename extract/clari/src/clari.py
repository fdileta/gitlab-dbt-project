...
def make_request(
    ...
    try:
        ...
        if response.status_code == 429:
            ...
            return make_request(
                request_type, url, headers, params, timeout, data,
...
def poll_job_status(
    ...
    while True:
        ...
        if status == "DONE":
            print(
                f'job_id {job_id} successfully completed, it is ready for export.'
            )
            return True

        if status in ['ABORTED', 'FAILED', 'CANCELLED']:
            raise Exception(
                f'job_id {job_id} failed to complete with {status} status'
            )
