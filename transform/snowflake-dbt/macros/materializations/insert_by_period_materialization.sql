...
{% materialization insert_by_period, default -%}
  ...
  {% for i in range(num_periods) -%} 
    ...
    {#% set tmp_table_sql = get_period_sql(target_cols_csv, sql, timestamp_field, period, start_timestamp, stop_timestamp, i) %#}
    ...
    {# set build_sql = get_insert_sql(target_relation, tmp_relation, target_columns) #}

    {% set build_sql = get_insert_period_sql(target_relation, target_columns, sql, timestamp_field, period, start_timestamp, stop_timestamp, i) %}
    ...
    {%- set msg = "Run completed for " ~ period ~ " " ~ ( i + 1 ) ~ " of " ~ num_periods ~ "; " ~ rows_inserted ~ " records inserted" -%}
    {{ dbt_utils.log_info(msg) }}
  {%- endfor %}
