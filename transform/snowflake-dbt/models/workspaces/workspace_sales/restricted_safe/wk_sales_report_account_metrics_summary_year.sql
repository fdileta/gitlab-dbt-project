...
-- Rolling 1 year Net ARR
  ...
  -- net_arr_ttm
    ...
    SELECT
      o.account_id,
      d.report_fiscal_year          AS report_fiscal_year,
      SUM(o.net_arr)                AS last_12m_closed_net_arr,   -- ttm_net_arr
      SUM(CASE
            WHEN  o.sales_qualified_source != 'Web Direct Generated'
              THEN o.net_arr
            ELSE 0
          END)          AS last_12m_closed_non_web_net_arr,  -- ttm_non_web_net_arr
      SUM(CASE
            WHEN o.sales_qualified_source = 'Web Direct Generated'
            THEN o.net_arr
            ELSE 0 END) AS last_12m_closed_web_direct_sourced_net_arr,  --ttm_web_direct_sourced_net_arr
      SUM(CASE
            WHEN o.sales_qualified_source = 'Channel Generated'
            THEN o.net_arr
            ELSE 0 END) AS last_12m_closed_channel_sourced_net_arr,  -- ttm_web_direct_sourced_net_arr
      SUM(CASE
            WHEN o.sales_qualified_source = 'SDR Generated'
            THEN o.net_arr
            ELSE 0 END) AS last_12m_closed_sdr_sourced_net_arr,  -- ttm_sdr_sourced_net_arr
      SUM(CASE
            WHEN o.sales_qualified_source = 'AE Generated'
            THEN o.net_arr
            ELSE 0 END) AS last_12m_closed_ae_sourced_net_arr,  -- ttm_ae_sourced_net_arr
      SUM(CASE
            WHEN o.is_eligible_churn_contraction_flag = 1
               THEN o.net_arr
            ELSE 0 END) AS last_12m_closed_churn_contraction_net_arr,  -- ttm_churn_contraction_net_arr

       -- FO year
        SUM(CASE
            WHEN o.order_type_live = '1. New - First Order'
            THEN o.net_arr
            ELSE 0 END) AS last_12m_closed_fo_net_arr,  -- ttm_fo_net_arr

        -- New Connected year
        SUM(CASE
            WHEN o.order_type_live = '2. New - Connected'
            THEN o.net_arr
            ELSE 0 END) AS last_12m_closed_new_connected_net_arr, -- ttm_new_connected_net_arr

        -- Growth year
        SUM(CASE
            WHEN o.order_type_live NOT IN ('2. New - Connected','1. New - First Order')
            THEN o.net_arr
            ELSE 0 END) AS last_12m_closed_growth_net_arr,   --ttm_growth_net_arr

        -- deal path direct year
        SUM(CASE
            WHEN o.deal_path != 'Channel'
            THEN o.net_arr
            ELSE 0 END) AS last_12m_closed_direct_net_arr,   --ttm_direct_net_arr

        -- deal path channel year
        SUM(CASE
            WHEN o.deal_path = 'Channel'
            THEN o.net_arr
            ELSE 0 END) AS last_12m_closed_channel_net_arr,   --ttm_channel_net_arr

        SUM (CASE
            WHEN o.is_won = 1
            THEN o.calculated_deal_count
            ELSE 0 END )   AS last_12m_closed_deal_count,  --ttm_deal_count

         SUM (CASE
            WHEN (o.is_won = 1
                  OR (o.is_renewal = 1 AND o.is_lost = 1))
            THEN o.calculated_deal_count
            ELSE 0 END )   AS last_12m_closed_trx_count,  -- ttm_trx_count
          ...
          SUM (CASE
            WHEN (o.is_won = 1
                ...
                AND ((o.is_renewal = 1 AND o.arr_basis > 5000)
                        OR o.net_arr > 5000)
            THEN o.calculated_deal_count
            ELSE 0 END )   AS last_12m_closed_trx_over_5k_count,   -- ttm_trx_over_5k_count
          ...
          SUM (CASE
            WHEN (o.is_won = 1
                ...
                AND ((o.is_renewal = 1 AND o.arr_basis > 10000)
                        OR o.net_arr > 10000)
            THEN o.calculated_deal_count
            ELSE 0 END )   AS last_12m_closed_trx_over_10k_count,  -- ttm_trx_over_10k_count
          ...
          SUM (CASE
            WHEN (o.is_won = 1
                ...
                AND ((o.is_renewal = 1 AND o.arr_basis > 50000)
                        OR o.net_arr > 50000)
            THEN o.calculated_deal_count
            ELSE 0 END )   AS last_12m_closed_trx_over_50k_count,  -- ttm_trx_over_50k_count

          SUM (CASE
            WHEN o.is_renewal = 1
            THEN o.calculated_deal_count
            ELSE 0 END )   AS last_12m_closed_renewal_deal_count,   -- ttm_renewal_deal_count

        SUM(CASE
            WHEN o.is_eligible_churn_contraction_flag = 1
                AND o.opportunity_category IN ('Standard','Internal Correction','Ramp Deal','Contract Reset','Contract Reset/Ramp Deal')
            THEN o.calculated_deal_count
            ELSE 0 END) AS last_12m_closed_churn_contraction_deal_count,  -- ttm_churn_contraction_deal_count
        ...
        SUM(CASE
            WHEN o.deal_path != 'Channel'
                AND o.is_won = 1
            THEN o.calculated_deal_count
            ELSE 0 END) AS last_12m_closed_direct_deal_count,  -- ttm_direct_deal_count
        ...
        SUM(CASE
            WHEN o.deal_path = 'Channel'
                AND o.is_won = 1
            THEN o.calculated_deal_count
            ELSE 0 END) AS last_12m_closed_channel_deal_count  -- ttm_channel_deal_count

    FROM sfdc_opportunity_xf AS o
    CROSS JOIN report_dates AS d
    WHERE
        o.close_date BETWEEN DATEADD(month, -12,DATE_TRUNC('month',d.report_month_date)) and DATE_TRUNC('month',d.report_month_date)
        AND (o.stage_name = 'Closed Won' OR (o.sales_type = 'Renewal' AND o.stage_name = '8-Closed Lost'))
        AND o.net_arr <> 0
    GROUP BY 1, 2
  ...
  -- LEAST(12,datediff(month,upa_owner.start_date,ak.report_month_date)) AS upa_months_in_year,
    ...
    mart_crm_account.parent_crm_account_lam_dev_count                  AS upa_aggregate_dev_count,
    ...
    COALESCE(net_arr_last_12m.last_12m_closed_net_arr,0)                       AS last_12m_closed_net_arr,
    COALESCE(net_arr_last_12m.last_12m_closed_non_web_net_arr,0)               AS last_12m_closed_non_web_net_arr,
    COALESCE(net_arr_last_12m.last_12m_closed_web_direct_sourced_net_arr,0)    AS last_12m_closed_web_direct_sourced_net_arr,
    COALESCE(net_arr_last_12m.last_12m_closed_channel_sourced_net_arr,0)       AS last_12m_closed_channel_sourced_net_arr,
    COALESCE(net_arr_last_12m.last_12m_closed_sdr_sourced_net_arr,0)           AS last_12m_closed_sdr_sourced_net_arr,
    COALESCE(net_arr_last_12m.last_12m_closed_ae_sourced_net_arr,0)            AS last_12m_closed_ae_sourced_net_arr,
    COALESCE(net_arr_last_12m.last_12m_closed_churn_contraction_net_arr,0)     AS last_12m_closed_churn_contraction_net_arr,
    COALESCE(net_arr_last_12m.last_12m_closed_fo_net_arr,0)                    AS last_12m_closed_fo_net_arr,
    COALESCE(net_arr_last_12m.last_12m_closed_new_connected_net_arr,0)         AS last_12m_closed_new_connected_net_arr,
    COALESCE(net_arr_last_12m.last_12m_closed_growth_net_arr,0)                AS last_12m_closed_growth_net_arr,
    COALESCE(net_arr_last_12m.last_12m_closed_deal_count,0)                    AS last_12m_closed_deal_count,
    COALESCE(net_arr_last_12m.last_12m_closed_direct_net_arr,0)                AS last_12m_closed_direct_net_arr,
    COALESCE(net_arr_last_12m.last_12m_closed_channel_net_arr,0)               AS last_12m_closed_channel_net_arr,
    COALESCE(net_arr_last_12m.last_12m_closed_channel_net_arr,0)  - COALESCE(net_arr_last_12m.last_12m_closed_channel_sourced_net_arr,0)   AS last_12m_closed_channel_co_sell_net_arr,
    COALESCE(net_arr_last_12m.last_12m_closed_direct_deal_count,0)             AS last_12m_closed_direct_deal_count,
    COALESCE(net_arr_last_12m.last_12m_closed_channel_deal_count,0)            AS last_12m_closed_channel_deal_count,
    COALESCE(net_arr_last_12m.last_12m_closed_churn_contraction_deal_count,0)  AS last_12m_closed_churn_contraction_deal_count,
    COALESCE(net_arr_last_12m.last_12m_closed_renewal_deal_count,0)            AS last_12m_closed_renewal_deal_count,
    COALESCE(net_arr_last_12m.last_12m_closed_trx_count,0)                     AS last_12m_closed_trx_count,
    COALESCE(net_arr_last_12m.last_12m_closed_trx_over_5k_count,0)             AS last_12m_closed_trx_over_5k_count,
    COALESCE(net_arr_last_12m.last_12m_closed_trx_over_10k_count,0)            AS last_12m_closed_trx_over_10k_count,
    COALESCE(net_arr_last_12m.last_12m_closed_trx_over_50k_count,0)            AS last_12m_closed_trx_over_50k_count,
  ...
  LEFT JOIN dim_crm_account AS dim_account
    ON a.account_id = dim_account.dim_crm_account_id
  LEFT JOIN mart_crm_account
    ON a.account_id = mart_crm_account.dim_crm_account_id
  ...
  LEFT JOIN last_12m_atr_base AS last_12m_atr_base
    ON a.account_id = last_12m_atr_base.account_id
  ...
  LEFT JOIN net_arr_last_12m
    ON a.account_id = net_arr_last_12m.account_id
  ...
  LEFT JOIN total_net_arr_fiscal AS net_arr_fiscal
  ...
  SELECT
   ...
   -- MAX(potential_carr_this_account)  AS potential_carr_this_account,
    ...
    -- rolling last 12 months bokked net arr
    SUM(last_12m_closed_net_arr)                      AS last_12m_closed_net_arr,
    SUM(last_12m_closed_non_web_net_arr)              AS last_12m_closed_non_web_net_arr,
    SUM(last_12m_closed_web_direct_sourced_net_arr)   AS last_12m_closed_web_direct_sourced_net_arr,
    SUM(last_12m_closed_channel_sourced_net_arr)      AS last_12m_closed_channel_sourced_net_arr,
    SUM(last_12m_closed_sdr_sourced_net_arr)          AS last_12m_closed_sdr_sourced_net_arr,
    SUM(last_12m_closed_ae_sourced_net_arr)           AS last_12m_closed_ae_sourced_net_arr,
    SUM(last_12m_closed_churn_contraction_net_arr)    AS last_12m_closed_churn_contraction_net_arr,
    SUM(last_12m_closed_fo_net_arr)                   AS last_12m_closed_fo_net_arr,
    SUM(last_12m_closed_new_connected_net_arr)        AS last_12m_closed_new_connected_net_arr,
    SUM(last_12m_closed_growth_net_arr)               AS last_12m_closed_growth_net_arr,
    SUM(last_12m_closed_deal_count)                   AS last_12m_closed_deal_count,
    SUM(last_12m_closed_direct_net_arr)               AS last_12m_closed_direct_net_arr,
    SUM(last_12m_closed_channel_net_arr)              AS last_12m_closed_channel_net_arr,
  ...
  SELECT
      ...
      -- JK 2022-07-04: populating LAM dev count for all accounts under the same parent
      -- CASE WHEN upa.upa_id = acc.account_id
      --      THEN  COALESCE(upa.lam_dev_count,0)
      --       ELSE 0 END                          AS lam_dev_count,
      upa.lam_dev_count AS lam_dev_count,
