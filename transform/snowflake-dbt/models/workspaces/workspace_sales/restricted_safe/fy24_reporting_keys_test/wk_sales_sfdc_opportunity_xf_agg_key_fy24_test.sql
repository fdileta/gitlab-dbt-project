{{ config(alias='sfdc_opportunity_xf_agg_key_fy24_test') }}
...
-- keys used for aggregated historical analysis
    ...
    SELECT *
    FROM {{ ref('wk_sales_report_agg_keys_fy24_test') }}
  ...
  WHERE date_actual = CURRENT_DATE
   ...
   SELECT
    ...
    -- for FY24 reporting keys
    CASE edm_opty.report_opportunity_user_segment
    ...
    CASE
      WHEN edm_opty.sales_qualified_source_name = 'Channel Generated'
        THEN 'Channel Generated'
      WHEN edm_opty.sales_qualified_source_name != 'Channel Generated'
        AND NOT(LOWER(resale_account.account_name) LIKE ANY ('%ibm%','%google%','%gcp%','%amazon%'))
        THEN 'Channel Co-Sell'
      WHEN edm_opty.sales_qualified_source_name != 'Channel Generated'
        AND LOWER(resale_account.account_name) LIKE ANY ('%ibm%','%google%','%gcp%','%amazon%')
        THEN 'Alliance Co-Sell'
      ELSE 'Direct'
    END                                               AS partner_category,   -- definition pending

    CASE
      WHEN LOWER(resale_account.account_name)LIKE '%ibm%'
        THEN 'IBM'
      WHEN LOWER(resale_account.account_name) LIKE ANY ('%google%','%gcp%')
        THEN 'GCP'
      WHEN LOWER(resale_account.account_name) LIKE '%amazon%'
        THEN 'AWS'
    ...
    SELECT
      ...
      agg_demo_keys.bu_geo_region_area_segment_rt,
      agg_demo_keys.key_bu,
      agg_demo_keys.key_rt,
    ...
    LEFT JOIN agg_demo_keys
      ON oppty_final.bu_geo_region_area_segment_rt_pc_ap = agg_demo_keys.bu_geo_region_area_segment_rt_pc_ap
