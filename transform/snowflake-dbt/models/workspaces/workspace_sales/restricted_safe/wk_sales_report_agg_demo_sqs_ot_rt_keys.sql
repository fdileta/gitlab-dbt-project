{{ config(alias='report_agg_demo_sqs_ot_rt_keys') }}
...
WITH sfdc_account_xf AS (
    ...
    SELECT 
        ...
        CASE
          ...
          WHEN lower(owner.role_name) like ('pooled%')
                  AND lower(opportunity.user_segment_stamped) IN ('smb','mid-market')
                  AND opportunity.order_type_stamped != '1. New - First Order'
              THEN 'Pooled'
          ...
          WHEN lower(owner.role_name) like ('named%')
                  ...
                  AND opportunity.order_type_stamped != '1. New - First Order'
              THEN 'Named'
          WHEN  opportunity.order_type_stamped IN ('2. New - Connected','4. Contraction','6. Churn - Final','5. Churn - Partial','3. Growth')
                AND lower(opportunity.user_segment_stamped) IN ('smb','mid-market')
              THEN 'Expansion'
          ELSE 'Other'
        END                                                            AS commercial_sal_category,  -- role_type
  ...
  SELECT         
        ...
        LOWER(commercial_sal_category)               AS role_type,
        ...
        LOWER(CONCAT(report_opportunity_user_segment, '-',report_opportunity_user_geo, '-',report_opportunity_user_region, '-',report_opportunity_user_area, '-', sales_qualified_source, '-', order_type_stamped, '-', commercial_sal_category))    AS report_user_segment_geo_region_area_sqs_ot_rt
  ...
  SELECT         
        ...
        LOWER(CONCAT(account_owner_user_segment,'-',account_owner_user_geo,'-',account_owner_user_region,'-',account_owner_user_area, '-', sales_qualified_source, '-', order_type_stamped, '-', commercial_sal_category))    AS report_user_segment_geo_region_area_sqs_ot_rt
  FROM sfdc_opportunity_xf
...
--------------
        report_opportunity_user_segment || '_' || role_type                          AS key_segment_rt,
...
--------------
        report_opportunity_user_segment || '_' || report_opportunity_user_geo || '_' ||  role_type                         AS key_segment_geo_rt,      
